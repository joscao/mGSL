...
#ifndef MGSL_RSTAT_QUANTILE_CLASS_H
#define MGSL_RSTAT_QUANTILE_CLASS_H
#pragma once
...
private:
	...
	double calc_psq(const std::array<double, 7>) const noexcept;
...
inline double quantile::calc_psq(const std::array<double, 7> data) const noexcept
{
	const double qp1 = data[0];
	const double q = data[1];
	const double qm1 = data[2];
	const double d = data[3];
	const double np1 = data[4];
	const double n = data[5];
	const double nm1 = data[6];
...
} // double quantile::calc_psq(const std::array<double, 7> data)
	...
	else
	{
		...
		for (int i = 1; i <= 3; ++i)
		{
			        ...
			        (d <= -1.0 && (npos[i - 1] - npos[i] < -1)))
			{
				...
				double qp = quantile::calc_psq(std::array<double, 7> {qp1, qi, qm1, (double) dsign, np1, ni, nm1});
...
#endif //MGSL_RSTAT_QUANTILE_CLASS_H